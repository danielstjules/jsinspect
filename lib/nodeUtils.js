...
nodeUtils.isCommonJS = function(nodes) {
  ...
  if (nodes[0].type === 'CallExpression' &&
      nodes[0].callee.name === 'require') {
    return true;
  }

  if (nodes[0].type === 'VariableDeclaration' && nodes[0].declarations) {
    for (var j = 0; j < nodes[0].declarations.length; j++) {
      if (nodes[0].declarations[j].type === 'VariableDeclarator' &&
          nodes[0].declarations[j].init &&
          nodes[0].declarations[j].init.type === 'CallExpression' &&
          nodes[0].declarations[j].init.callee.name === 'require') {
        return true;
      }
    }
  }
