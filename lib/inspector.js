...
class Inspector extends EventEmitter {
   ...
   * to be an array of string paths. Also accepts an options object any
   ...
   * indicates whether or not the nodes in a match should also have matchin
   ...
   * conforming to the ESTree spec. Acorn's loose parser is used to handle files
   * with potential syntax errors. The parser is invoked with ES6 support
  ...
  _walk(node, fn) {
    nodeUtils.walkSubtrees(node, (node, parent, ancestors) => {
      ...
      var children = nodeUtils.getBFSTraversal(node, this._threshold).slice(1);
      ...
      if (children.length === this._threshold - 1) {
  ...
  _prune(key, nodes) {
    ...
    for (i = 0; i < length; i++) {
      ...
      targetNodes = nodeUtils
        .getBFSTraversal(nodes[i], 10 * this._threshold)
        .slice(1);
