...
Inspector.prototype._analyze = function() {
  ...
  emitMatches = function(type, object) {
    ...
    for (key in object) {
      ...
      match = new Match(key, type, object[key].slice(0));
      if (self._diff) {
...
Inspector.prototype._insertIdentical = function(nodes) {
  ...
  // Store keys on the parent, mapping to the inner-most node
  if (!nodes[0].keys._identical) {
    nodes[0].keys._identical = {};
  }

  nodes[0].keys._identical[key] = nodes[nodes.length - 1];
};
...
Inspector.prototype._insertFuzzy = function(nodes) {
  ...
  for (i = 1; i < maxIndex; i++) {
    ...
    // Store keys on parents, mapping to the inner-most nodes
    if (!nodes[0].keys._fuzzy) {
      nodes[0].keys._fuzzy = {};
    }

    nodes[0].keys._fuzzy[key] = nodes[nodes.length - 1];
  }
};
