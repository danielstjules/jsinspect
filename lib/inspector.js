...
function Inspector(filePaths, opts) {
  ...
  this._literals    = opts.literals    || Inspector.defaultLiterals;
...
/**
 * A simple object emulating a map-like structure for defining the default
 * types of literals to match against.
 *
 * @var {object}
 */
Inspector.defaultLiterals = {
  boolean: true,
  number:  true,
  string:  true
};
...
Inspector.prototype._analyze = function() {
  ...
  for (key in this._hash) {
    ...
    if (this._identifiers) {
      nodes = nodeUtils.groupByMatchingIds(key, nodes, this._identifiers);
    }
    if (this._literals) {
