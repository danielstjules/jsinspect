 ...
 * number of nodes to analyze, and diff enables the generation of diffs for any
 * outputted matches. Identifiers indicates whether or not the nodes in a match
 * should also have matching identifiers. An instance of Inspector emits the
 * following events: start, match and end.
 *
 * @constructor
  ...
  recurse = function(node, state, override) {
    if (i++ && node !== prev) {
      ...
      if (!parent || (!parent.numChildren && parent.numChildren !== 0) ||
        ...
        // Ignore AMD and CommonsJS define and require expressions
...
Inspector.prototype._prune = function(key, nodes) {
  ...
  for (i = 0; i < length; i++) {
    ...
    targetNodes = nodeUtils.getNestedChildren(nodes[i], 10 * this._threshold);
