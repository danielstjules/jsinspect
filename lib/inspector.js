 ...
 * If literal or identifier matching is enabled, then the node arrays stored in
 * the parents are also compared to determine the match.
 *
 * @private
 *
 * @fires Inspector#match
...
Inspector.prototype._analyze = function() {
  ...
  for (key in this._hash) {
    ...
    if (this._identifiers) {
      nodes = this._filterMatchingIds(key, nodes);
    }
    if (this._literals) {
      nodes = this._filterMatchingLiterals(key, nodes);
    }
    if (nodes.length < 2) {
      continue;
    }

    match = new Match(key, !!this._distance, nodes);
    if (this._diff) {
      match.generateDiffs(this._fileContents);
    }

    this.emit('match', match);
    this._prune(key, nodes);
  }
};

Inspector.prototype._filterMatchingIds = function(key, nodes) {
  var filtered, i;
...
Inspector.prototype._filterMatchingLiterals = function(key, nodes) {
  return nodes;
};
