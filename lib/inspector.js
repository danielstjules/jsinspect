 ...
 * an array of more than a single node indicates a match. A match results in
 * the relevant child nodes being removed from any future results. This pruning
 * ensures that we only include the greatest common parent in a set of matches.
 * Pruning of an identical match results in the possible removal of its nodes
 * from any fuzzy matches as well. If literal or identifier matching is enabled,
 * then the node arrays stored in the parents are also compared to determine
 * the match.
 *
 * @private
 *
...
Inspector.prototype._analyze = function() {
  ...
  emitMatches = function(object, fuzzy) {
    ...
    for (key in object) {
      ...
      nodes = object[key].slice(0);
      if (self._identifiers) {
        self._filterMatchingIds(nodes);
      }
      if (self._literals) {
        self._filterMatchingLiterals(nodes);
      }
      if (nodes.length < 2) {
        continue;
      }

      match = new Match(key, fuzzy, nodes);
      if (self._diff) {
        match.generateDiffs(self._fileContents);
      }

      self.emit('match', match);
      self._prune(key, nodes, fuzzy);
    }
  };
...
Inspector.prototype._filterMatchingIds = function(nodes) {
  return nodes;
};

Inspector.prototype._filterMatchingLiterals = function(nodes) {
  return nodes;
};
...
Inspector.prototype._walk = function(node, fn) {
  ...
  recurse = function(node, state, override) {
    ...
    if (i++ && node !== prev) {
      ...
      length = state.length;
      if (length >= threshold) {
        for (var j = 0; j < length - threshold + 1; j++) {
          fn(state.slice(j, length));
        }
      }
    }
...
Inspector.prototype._insertIdentical = function(nodes) {
  ...
  if (!this._identical[key]) {
    this._identical[key] = [];
  } else if (this._identical[key].indexOf(nodes[0]) !== -1) {
    return;
  }
...
Inspector.prototype._insertFuzzy = function(nodes) {
  ...
  for (i = 1; i < maxIndex; i++) {
    ...
    if (!this._fuzzy[key]) {
      this._fuzzy[key] = [];
    } else if (this._fuzzy[key].indexOf(nodes[0]) !== -1) {
      return;
    }
