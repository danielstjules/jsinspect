...
BaseReporter.prototype._getFormattedDiff = function(diff) {
  ...
  for (i = 0; i < diff.length; i++) {
    ...
    if (diffLength > 80) {
      return 'Diff suppressed as it surpasses 80 lines';
    }
 ...
 * located, as well as the lines on which the node exists, up until its
 * innermost matched child.
 *
 * @param   {Node}   node The node from which to get a formatted source
 * @param   {string} key  The match key
 * @returns {string} The formatted string
 */
BaseReporter.prototype._getFormattedSource = function(node, key) {
  return node.loc.source + ':' + node.loc.start.line + ',' +
    node.loc.end.line;
};
