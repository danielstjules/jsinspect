...
function BaseReporter(inspector, opts) {
  ...
  this._writeStream = opts.writeStream || process.stdout;
  this._registerListener();

  inspector.on('end', function() {
    if (self._writeStream !== process.stdout) {
      // give the reporter a chance to write its last chunk by defering end to next tick
...
BaseReporter.prototype._registerListener = function() {
  ...
  this._inspector.on('match', function(match) {
    self._found++;
    self._writeStream.write(self._getOutput(match));
  });
};
...
BaseReporter.prototype._registerSummary = function() {
  ...
  this._inspector.on('end', function() {
    ...
    self._writeStream.write(summary + '\n');
  });
};
